                ...
                if (md5ParserFunc) {
                    request(md5Url, function(error, response, body) {
                        if (!error && response.statusCode == 200) {
                            ...
                            if (result) {
                                console.log(md5Url + ' => ' + result);
                            } else {
                                console.error(md5Url + ' => Error while parsing MD5');
                            }
                        } else {
                            console.error(md5Url + ' => Error while requesting MD5 URL.');
                        }
                    });
                } else {
                    console.warn(md5Url + ' => No MD5 parser found.');
                }
            });
            console.log();
        }
